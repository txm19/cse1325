...
int main(int argc, char* argv[]) {
    ...
    // -------------------------------------------------------------------------
    vector <<= 1;   // Subscript operator []
    Time t_subscript{9, 30, 0};

    if (t_subscript[0] != 9 || t_subscript[1] != 30 || t_subscript[2] != 0) {
        std::cerr << "FAIL: Subscript operator [] for Time object" << std::endl;
        result |= vector;
    }
